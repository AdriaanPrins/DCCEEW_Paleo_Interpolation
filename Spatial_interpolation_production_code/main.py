...
# ==================================================
def run_production_interpolation(current_date, target_variable,target_model, df_train_daily, df_grid, shapes, full_directory):
...
def run_pp_daily_task(task_args):
    d, df_train_daily, df_grid, shapes, full_directory, variable, model = task_args
    try:
        run_production_interpolation(
            current_date=d, target_variable=variable, target_model = model, df_train_daily=df_train_daily, 
            df_grid=df_grid, shapes=shapes, full_directory=full_directory
        )
    except Exception as e:
        print(f"❌ Failed on Date: {d}, Variable: {variable}. Error: {e}")
...
def sum_rasters_memory_safe(input_files, output_path):
    ...
    with rasterio.open(valid_files[0]) as src:
        ...
        meta.update({"height": win_height, "width": win_width, "transform": win_transform, "dtype": rasterio.float32,"compress": "lzw",  "predictor": 3, "tiled": True })
...
def run_local_aggregations(output_folder, decade, years_to_process, variable, model):
    ...
    for year in years_to_process:
        ...
        daily_dir = os.path.join(output_folder, f'{decade}/{year}/Daily_{variable}_{model}/')
        monthly_dir = os.path.join(output_folder, f'{decade}/{year}/Monthly_{variable}_{model}/')
        yearly_dir = os.path.join(output_folder, f'{decade}/{year}/Yearly_{variable}_{model}/')
        ...
        for model, months in monthly_groups.items():
            ...
            for month, files in sorted(months.items()):
                month_filename = f"{variable}_{model}_{year_str}_{month}_Total.tif"
            ...
            if yearly_files:
                year_filename = f"{variable}_{model}_{year_str}_Total.tif"
...
if __name__ == '__main__':
    ...
    # 1. Pull your new environment variables
    task_start_date = os.environ.get("START_DATE")
    task_end_date = os.environ.get("END_DATE")
    model = os.environ.get("MODEL")
    target_variable = os.environ.get("VARIABLE")
    current_run_date = os.environ.get("CURRENT_RUN_DATE")
    selected_datatable = os.environ.get("DATASET")

    # Safety check
    if not all([task_start_date, task_end_date, model, target_variable]):
        print("Error: Missing required environment variables.", file=sys.stderr)
        sys.exit(1)

    # 2. Calculate the decades and years mathematically from the START_DATE
    start_year = int(task_start_date.split("-")[0])
    start_month = int(task_start_date.split("-")[1])
    start_day = int(task_start_date.split("-")[2])
    
    end_year = int(task_end_date.split("-")[0])
    end_month = int(task_end_date.split("-")[1])
    end_day = int(task_end_date.split("-")[2])
    ...
    query_grid = """
        ...
        """
    df_grid = client.query(query_grid).to_dataframe().sort_values(['Latitude', 'Longitude'], ascending=[False, True]).dropna()
    ...
    print(f"\n{'='*50}")
    print(f"BEGINNING FULL PIPELINE FOR DECADE: {decade}s")
    print(f"{'='*50}")

    print(f"Fetching BigQuery data for {decade}s...")
    query = f"""
        SELECT Station_Region_ID, Decade, Year, Month, Day, {target_variable}
        FROM `{selected_datatable}`
        WHERE Decade = {decade} And {target_variable} IS NOT NULL
    """
   ...
   # 3. Build your sequential dates safely (Respecting start/end months and days)
    sequential_dates_three_years = []
    
    for year in years_to_process:
        # Determine the start month/day for this specific year iteration
        if year == start_year:
            m_start, d_start = start_month, start_day
        else:
            m_start, d_start = 1, 1
            
        # Determine the end month/day for this specific year iteration
        if year == end_year:
            m_end, d_end = end_month, end_day
        else:
            m_end, d_end = 12, 31
            
        # Safe year handling for datetime module (which doesn't support year 0)
        safe_year = 1 if year == 0 else year
        
        loop_start = date(safe_year, m_start, d_start)
        loop_end = date(safe_year, m_end, d_end)
        total_days = (loop_end - loop_start).days + 1
        
        sequential_dates = []
        for offset in range(total_days):
            current_date = loop_start + timedelta(days=offset)
            date_str = current_date.strftime("%d/%m/%Y")
            
            # If we are processing actual year 0, swap the '0001' back to '0000'
            if year == 0:
                date_str = date_str[:-1] + '0'
                
            sequential_dates.append(date_str)
    ...
    for year in years_to_process:
        ...
        daily_directory = os.path.join(output_folder, f'{decade}/{year}/Daily_{target_variable}_{model}/')
    ...
    for d in sequential_dates_three_years:
        ...
        daily_directory = os.path.join(output_folder, f'{decade}/{int(year_str)}/Daily_{target_variable}_{model}/')
        ...
        if not df_train_daily.empty:
            tasks.append((d, df_train_daily, df_grid, shapes, daily_directory, target_variable, model))
    ...
    safe_cores = max(1, int(total_cores * 0.7)) 
    ...
    run_local_aggregations(output_folder, decade, years_to_process, target_variable, model)

    bucket_name = 'dcceew-input-data'
    safe_run_date = current_run_date.strip().replace(" ", "_").replace(":", "-")
    gcs_prefix = f"Processing_Runs/Run_{model}_{target_variable}_{safe_run_date}"
